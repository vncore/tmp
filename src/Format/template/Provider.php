...
function sc_process_css_default($storeId = null) {
        ...
        \Vncore\Core\Front\Models\ShopStoreCss::insert(['css' => $cssContent, 'store_id' => $storeId, 'template' => sc_template_info()['configKey']]);
    }
}
...
function sc_template_uninstall_store($storeId = null) {
        if ($storeId) {
        \Vncore\Core\Admin\Models\AdminStoreBlockContent::where('template', sc_template_info()['configKey'])
        ...
        \Vncore\Core\Front\Models\ShopStoreCss::where('template', sc_template_info()['configKey'])
        ->where('store_id', $storeId)
        ->delete();
    } else {
        ...
        \Vncore\Core\Admin\Models\AdminStoreBlockContent::where('template', sc_template_info()['configKey'])
            ->delete();
        $idBanners = \Vncore\Core\Front\Models\ShopBanner::where('title', 'like', '%('.sc_template_info()['configKey'].')%')
            ->pluck('id');
        if ($idBanners) {
            \Vncore\Core\Front\Models\ShopBannerStore::whereIn('banner_id', $idBanners)
            ->delete();
