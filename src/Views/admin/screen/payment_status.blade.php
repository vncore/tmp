    ...
    <div class="card">
      <div class="card-header">
        ...
        <div class="btn-group float-right" style="margin-right: 5px">
            <a href="{{ vc_route_admin('admin_payment_status.index') }}" class="btn  btn-flat btn-default" title="List"><i class="fa fa-list"></i><span class="hidden-xs"> {{ vc_language_render('admin.back_list') }}</span></a>
        </div>
      @endif
      </div>
      <!-- /.card-header -->
      ...
      <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main">
        <div class="card-body">
          <div class="form-group row {{ $errors->has('name') ? ' text-red' : '' }}">
            <label for="name" class="col-sm-2 col-form-label">{{ vc_language_render('admin.payment_status.name') }}</label>
            <div class="col-sm-10 ">
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.error == 1){
                      ...
                      window.location.replace('{{ vc_route_admin('admin_payment_status.index') }}');
                    }
