...
namespace Vncore\Core\Library\ShoppingCart;

use Vncore\Core\Library\ShoppingCart\Exceptions\CartAlreadyStoredException;
use Vncore\Core\Library\ShoppingCart\Exceptions\UnknownModelException;
use Closure;
...
class Cart
{
     ...
     * @return \Vncore\Core\Library\ShoppingCart\Cart
     */
     ...
     * @return \Vncore\Core\Library\ShoppingCart\CartItem
     */
    ...
    public function content()
    {
        ...
        foreach ($content as $key => $item) {
            $product = \Vncore\Core\Front\Models\ShopProduct::where('id', $item->id)
    ...
    public static function getListCart($instance = self::DEFAULT_INSTANCE)
    {
        ...
        if ($cart->count()) {
            foreach ($cart->content() as $key => $item) {
                $product = \Vncore\Core\Front\Models\ShopProduct::find($item->id);
                if ($product) {
                    $arrCart['items'][] = [
                        'id'        => $item->id,
                        'rowId'     => $item->rowId,
