...
if (!function_exists('sc_order_process_after_success') && !in_array('sc_order_process_after_success', config('helper_except', []))) {
    function sc_order_process_after_success(string $orderID = ""):array
    {
        ...
        if ((sc_config('order_success_to_admin') || sc_config('order_success_to_customer')) && sc_config('email_action_mode')) {
            $data = \Vncore\Core\Front\Models\ShopOrder::with('details')->find($orderID)->toArray();
            $checkContent = (new \Vncore\Core\Front\Models\ShopEmailTemplate)->where('group', 'order_success_to_admin')->where('status', 1)->first();
            $checkContentCustomer = (new \Vncore\Core\Front\Models\ShopEmailTemplate)->where('group', 'order_success_to_customer')->where('status', 1)->first();
            if ($checkContent || $checkContentCustomer) {
                ...
                foreach ($data['details'] as $key => $detail) {
                    $product = (new \Vncore\Core\Front\Models\ShopProduct)->getDetail($detail['product_id']);
